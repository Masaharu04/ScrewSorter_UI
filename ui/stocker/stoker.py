...
class StockerApp:
    def __init__(self, parent):
        ...
        ctk.CTkLabel(self.stocker_frame, text="ストッカーレール残量レベル", font=("Arial", 18, "bold"), text_color="#ffffff").pack(pady=(0, 0))
        ...
        for i, (text, value) in enumerate(zip(self.display_labels, self.stocker_values)):
            ...
            self.canvas[i] = ctk.CTkCanvas(self.circle_frame[i], width=120, height=120, bg="#2b2b2b", highlightthickness=0)
            self.canvas[i].pack()

            # 背景の円
            self.canvas[i].create_oval(10, 10, 110, 110, fill="#3A3A3A", outline="")
            
            # 追加: 色を取得するための関数を呼び出す
            text_value, text_color = self.get_text_value(value) 
            color = text_color

            arc = self.canvas[i].create_arc(10, 10, 110, 110, start=90, extent=-360 * value, fill=color, outline="")
           # self.circles.append([self.canvas, arc])
            self.circles[i] = [self.canvas[i], arc]
    

            # 中央の円（くり抜き効果）
            self.canvas[i].create_oval(35, 35, 85, 85, fill="#2b2b2b", outline="")
    ...
    def update(self, photo):
        ...
        for i, (text, value) in enumerate(zip(self.display_labels, self.stocker_values)):
            ...
            arc = self.canvas[i].create_arc(10, 10, 110, 110, start=90, extent=-360 * photo[i], fill=color, outline="")
           # self.circles.append([self.canvas, arc])
            self.circles[i] = [self.canvas[i], arc]
            ...
            self.value_labels[i].configure(text=percentage_value, text_color=text_color)
            # self.value_labels[i] = ctk.CTkLabel(self.circle_frame[i], text=percentage_value, font=("Arial", 20, "bold"), text_color=text_color)
            # self.value_labels[i].place(relx=0.5, rely=0.5, anchor="center")
           # self.value_labels.append(Svalue_label) 
            self.label_buf[i].configure(text=text)
            # self.label_buf[i] = ctk.CTkLabel(self.stocker_grid, text=text, font=("Arial", 14), text_color="#cccccc", wraplength=120)
            # self.label_buf[i].grid(row=1, column=i, padx=5, pady=(5, 0), sticky="nsew")
