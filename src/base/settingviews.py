...
class MaintenanceView:
    def __init__(self, master, on_close):
        ...
        self.selected_labels = [None] * 4  # 選択されたラベルを保持するリスト
    ...
    def _create_stocker_selection(self, parent):
        ...
        for i in range(4):
            self.selected_labels[i] = ctk.StringVar(value=stocker_labels[0])  # デフォルト選択
            label_frame = ctk.CTkFrame(parent)
            label_frame.pack(anchor="w", padx=20, pady=5)
            ctk.CTkLabel(label_frame, text=f"{chr(65 + i)}:").pack(side="left")  # A, B, C, Dのラベル
    ...
    def confirm_selection(self):
        # 選択されたストッカーを表示
        selected_values = [label.get() for label in self.selected_labels]
